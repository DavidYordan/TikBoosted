...
def create_directory(path):
    final_path = os.path.join(path, 'TikBoosted')
...
def extract_resources(source_dir, target_dir):
    ...
    for resource_path in resource_paths:
        ...
        if resource_path == 'dist':
            shutil.copy(
                os.path.join(full_source_path, 'TikBoosted.exe'),
                os.path.join(target_dir, 'TikBoosted.exe')
            )
        else:
...
def main():
    ...
    exe_path = os.path.join(working_dir, 'TikBoosted.exe')
    
    create_shortcut_with_vbscript(working_dir, exe_path, 'TikBoosted', get_desktop_path())
