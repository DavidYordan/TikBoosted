...
class UsersAsiaTab(QWidget):
    def __init__(
        ...
        self.headers = {'Token': ''}
        self.url_base = 'https://admin.asia.reelshors.com'
        self.user = 'UsersAsiaTab'
        self.setup_ui()
        ...
        try:
            async with async_playwright() as p:
                ...
                if token:
                    ...
                    Globals.database_operation_signal.emit('upsert',{
                        'table_name': 'tokens',
                        'columns': ['name', 'token', 'expire'],
                        'values': ['asia_token', token, int(time.time()+token_response.get('expire', 0))],
                        'unique_columns': ['name']
                    }, None)
                else:
    ...
    async def update_users_worker(self):
        ...
        Globals._WS.set_progress_bar_title_signal.emit('Updating users')
        ...
        while True:
            try:
                ...
                res = requests.get(f'{self.url_base}/sqx_fast/user/selectUserList?page={page}&limit={500}', headers=self.headers)
    ...
    def setup_ui(self):
        ...
        button_update = QPushButton('Update Users')
        Globals.components.append(button_update)
        button_update.clicked.connect(self.update_users)
    ...
    def get_token_from_database(self):
        if not self.headers['Token']:
            res = Globals._SQL.read('tokens', condition='name="asia_token"')
            if not res:
